...
module "data_embedding_handler_lambda" {
  ...
  vpc_subnet_ids         = var.lambda_config.data_embedding_handler.in_vpc ? module.vpc.private_subnets : []
  vpc_security_group_ids = var.lambda_config.data_embedding_handler.in_vpc ? [aws_security_group.lambda_security_group.id] : []
  attach_network_policy  = var.lambda_config.data_embedding_handler.in_vpc
...
data "aws_ecr_repository" "query_handler_ecr_lambda" {
  name = "${var.lambda_config.query_handler.name}-lambda"
}

data "aws_ecr_image" "latest_query_handler_lambda_docker_image" {
  repository_name = data.aws_ecr_repository.query_handler_ecr_lambda.name
  most_recent     = true
}
...
module "query_handler_lambda" {
  ...
  function_name              = var.lambda_config.query_handler.name
  description                = var.lambda_config.query_handler.description
  image_config_command       = var.lambda_config.query_handler.image_config_command
  runtime                    = var.lambda_config.query_handler.runtime
  publish                    = true
  create_lambda_function_url = var.lambda_config.query_handler.function_url

  timeout                        = var.lambda_config.query_handler.timeout
  memory_size                    = var.lambda_config.query_handler.memory_size
  reserved_concurrent_executions = var.lambda_config.query_handler.reserved_concurrent_executions

  create_package = false
  image_uri      = "${data.aws_ecr_repository.query_handler_ecr_lambda.repository_url}:${data.aws_ecr_image.latest_query_handler_lambda_docker_image.image_tags[0]}"
  package_type   = var.lambda_config.query_handler.package_type
  ...
  vpc_security_group_ids = var.lambda_config.query_handler.in_vpc ? [aws_security_group.lambda_security_group.id] : []
  attach_network_policy  = var.lambda_config.query_handler.in_vpc
